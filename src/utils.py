...
def get_collection_as_dataframe(database_name:str,collection_name:str):
    try:
        logging.info(f"reading the data from the database from mongoDB")
        df=pd.DataFrame(list(mongo_client[database_name][collection_name].find()))
        logging.info(f"Data loaded from mongoDB sucessfully with rows {df.shape[0]} and columns{df.shape[1]}")
        if "_id" in df.columns:
            logging.info("dropping the _id column")
            df.drop("_id",axis=1,inplace=True)
            logging.info(f"_id columns droped")
        return df
...
def load_object(file_path:str):
    try:
        if not os.path.exists(file_path):
            raise CustomException(f"File not found at {file_path}")
        with open(file_path,"rb") as file_obj:
